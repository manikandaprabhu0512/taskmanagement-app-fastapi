...
from src.utils.helper import isAuthenticated

task_routes = APIRouter(prefix="/tasks")

@task_routes.post("/create-task", response_model=TaskResponseSchema, status_code=status.HTTP_201_CREATED)
def create_tasks(body:TaskSchema, db:Session = Depends(get_db), user:UserModel = Depends(isAuthenticated)):
    return controller.create_task(body, db, user)

@task_routes.get("/", response_model=List[TaskResponseSchema], status_code=status.HTTP_200_OK)
def get_tasks(db:Session = Depends(get_db), user:UserModel = Depends(isAuthenticated)):
...
def get_task(id:int, db:Session = Depends(get_db), user:UserModel = Depends(isAuthenticated)):
...
def get_task(body:TaskSchema, id:int, db:Session = Depends(get_db), user:UserModel = Depends(isAuthenticated)):
